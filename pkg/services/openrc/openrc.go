...
func (h *OpenRC) Enable(ctx context.Context, name string) error {
	...
	defer span.End()
	return execs.SimpleRunCommand("/sbin/rc-update", "add", name)
}

func (h *OpenRC) Disable(ctx context.Context, name string) error {
	_, span := h.tracer.Start(ctx, "Disable")
	defer span.End()
	return execs.SimpleRunCommand("/sbin/rc-update", "del", name)
}
...
func (h *OpenRC) Status(ctx context.Context, name string) (services.ServiceStatus, error) {
	...
	output, exit, err := execs.RunCommand("/bin/rc-status", "sysinit", "-Cqf", "ini")
	if exit == 0 {
